...
use rocket::Data;
use rocket_multipart_form_data::{MultipartFormDataOptions, MultipartFormData, MultipartFormDataField, RawField, TextField};
...
fn upload_avatar_pictures(content_type: &ContentType, data: Data) -> Result<String, OError> {
	...
	options.allowed_fields.push(MultipartFormDataField::text("id"));
	options.allowed_fields.push(MultipartFormDataField::raw("pic1").size_limit(1_000_000));
	options.allowed_fields.push(MultipartFormDataField::raw("pic2").size_limit(1_000_000));
	...
	if id.is_none() || pic1.is_none() || pic2.is_none() {
		return Err(OError::BadRequest(new_ejson("Please fill out all of the fields!")));
	}
	...
	storage.put(format!("{}-1.png", single_id.text), either::Either::Right(single_pic1.raw.clone()))?;
	storage.put(format!("{}-2.png", single_id.text), either::Either::Right(single_pic2.raw.clone()))?;
	Ok("Successfully uploaded images!".into())
}

pub fn mount(rocket: rocket::Rocket) -> rocket::Rocket {
	rocket.mount("/api/avatar/new", routes![create_avatar, upload_avatar_pictures])
}
