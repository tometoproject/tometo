...
pub fn synthesize(
	...
	if cfg.get::<String>("otemot.speech").unwrap() == "google" {
		...
		let mut res = Request::post(format!(
		...
		let mut buffer = BufWriter::new(fs::File::create(format!(
			"otemot/gentts/{}/temp.ogg",
			name
		))?);
		...
		Command::new("espeak")
			.args(&[
				"-p",
				&scale_pitch.to_string(),
				"-s",
				&scale_speed.to_string(),
				"-w",
				&format!("otemot/gentts/{}/temp.wav", name),
				&text,
			])
			.output()?;
		Command::new("ffmpeg")
	...
	fs::write(
		...
		&text.split(' ').collect::<Vec<&str>>().join("\n"),
	)?;
	Command::new("python3")
		.args(&[
			"-m",
